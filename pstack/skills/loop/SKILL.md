...
- On Cursor with `cursor-subscriptions-subscribe_timer`, use **Subscription timer (Cursor cloud)**. A local `sleep` loop does not work in Cursor cloud.
- On Cursor with monitored shell output and `notify_on_output`, use **Monitored shell output (Cursor local)**.
- On another harness, use only the wake mechanism documented in its active profile and exposed by its tools. An active-session wait requires the agent to stay active and resume the work after each wait. Ordinary background shell output does not imply a future agent wake.
- If no wake mechanism is available, run the prompt once and report that recurring execution could not be armed.
...
# Subscription timer (Cursor cloud)
...
# Monitored shell output (Cursor local)
