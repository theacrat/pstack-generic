---
name: automate
description: Use this skill to create agent automations in the active harness.
environments:
  - local
---
# Create Automation (Interactive)

Read [harness compatibility](../../docs/harnesses.md) and only the active profile before choosing tools or paths.

## Harness scope

The Cursor Automations procedure below applies only when Cursor Automations is the requested surface. Its editor, MCP metadata paths, authentication flow, proto fields, cron semantics and trigger tables are Cursor-specific; do not send those payloads to another harness.

For another explicitly requested automation surface, use the active profile's native scheduling alternative: Codex Automations when exposed, Claude Code or Grok Build scheduled tasks for session loops, Grok Bot routines, or an already configured scheduler for Antigravity, OpenCode, Gemini CLI and Copilot. A session loop does not provide durable scheduling or event triggers. Check the actual tool's supported triggers, timezone, persistence and authentication before proceeding; if it cannot fulfil the request, prepare the draft and report the missing capability without claiming it is active.

On that surface, preserve intent capture, scoped integration discovery, completeness checks and the reviewed draft table: name/description, trigger, tools, instructions, resolved settings and deferred fields. Use its documented configuration and finish flow, including its approval requirements. Resolve MCP identities and authentication from its live catalogue. Verify the saved configuration and a test run when supported. Do not apply the Cursor-only Agents Window requirement, backend prohibition or wire schema to a different service. This skill remains creation-only; route updates to that service's supported edit flow. Stop here after completing the alternative flow.
