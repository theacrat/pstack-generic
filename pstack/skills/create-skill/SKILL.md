...
This skill guides you through creating effective Agent Skills for the active harness. Skills are markdown files that teach the agent how to perform specific tasks: reviewing PRs using team standards, generating commit messages in a preferred format, querying database schemas, or any specialized workflow.
...
2. **Target location**: Should this be a personal skill or project skill in the active harness's skill directory?
...
If you need clarification, use the active harness's structured question tool when available:

```
Example structured question usage:
- "Where should this skill be stored?" with options like ["Personal", "Project"]
- "Should this skill include executable scripts?" with options like ["Yes", "No"]
```

If a structured question tool is not available, ask these questions conversationally.
...
| Type | Path | Scope |
|------|------|-------|
| Personal | Active profile's personal skill directory, then `skill-name/` | Available across all your projects |
| Project | Active profile's project skill directory, then `skill-name/` | Shared with anyone using the repository |

**IMPORTANT**: Never create skills in a harness-managed built-in directory. For Cursor, this includes `~/.cursor/skills-cursor/`.
...
When the active harness supports `disable-model-invocation`, default it to `true` so the skill only loads when named explicitly. Omit it only when the agent should auto-invoke from ambient context. Follow the active profile for supported frontmatter.
...
If you have access to a structured question tool, use it for efficient structured gathering. Otherwise, ask conversationally.
