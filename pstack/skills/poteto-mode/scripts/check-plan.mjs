...
const LANES = /Ten lanes on `[^`\s]+` at the PR head/;
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_MARKERS = [/(?:\/|\$)(?:pstack:)?goal\b/, /git show origin\/main:|Read installed plugin files/, /30[- ]minute/, "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks ten lanes on a named model at the PR head`);
