...
#
# Usage: worktree-audit.sh [repo-path] [workspace-transcripts-dir]   (defaults to the current repo)
set -u
...
# Pass only a workspace-scoped transcript directory resolved by the harness profile.
transcripts="${2:-}"
now=$(date +%s)
