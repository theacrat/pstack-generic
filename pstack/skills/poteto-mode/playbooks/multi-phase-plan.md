...
6. For an installed plugin, replace the `git show origin/main:pstack/...` checkboxes below with `Read installed plugin files` and the resolved file paths. Run `node pstack/skills/poteto-mode/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill).
...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use `control-ui` bundled with pstack. CLIs and TUIs use `control-cli` bundled with pstack. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
Each live lane runs on its own cloud VM at the PR head. Drive through `control-ui` or `control-cli` bundled with pstack.
