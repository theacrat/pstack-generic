...
See [installation instructions](./docs/install.md) for Cursor, Codex, Claude Code, Gemini CLI, GitHub Copilot, Antigravity, Grok, and OpenCode. Cursor: `/add-plugin pstack`.
...
that's it. model IDs below are Cursor defaults; [the harness mapping](./docs/harnesses.md) resolves available models or inherits the parent. the other skills are situational; the mode skill uses them for you as needed. out of the box the mode splits work by model strength: code delegates (feature, refactoring, bug fix, perf, hillclimb) go to grok, while the hardest changes, prose, and judgment go to fable 5.1. the default panel is fable 5.1 / sol / grok / opus 5. [`/setup-pstack`](./skills/setup-pstack/SKILL.md) changes any of it.
...
[`/poteto-mode`](./skills/poteto-mode/SKILL.md) works extremely well with the bundled `/loop` skill. you can make your agent work for many hours without sacrificing rigor.
...
Agent names and spawn parameters below use Cursor notation; [the harness mapping](./docs/harnesses.md) resolves them for your harness.
...
the things `poteto-mode` references are bundled:

- `/deslop` and the `deslop` skill are copied from `cursor-team-kit`.
- `control-cli` (for CLIs and TUIs), `control-ui` (for browser, Electron, web), and its `verify-this` dependency are copied from `cursor-team-kit` too.
- `/create-skill`, `/goal`, `/loop`, and `/automate` are copied from the Cursor built-ins. the [babysit playbook](./skills/poteto-mode/playbooks/babysit.md) handles pr-status requests.

see [dependency provenance](./docs/dependencies.md) for sources and update instructions.

## why are there no planning skills?

your harness may already have a plan mode which works great with pstack. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`/poteto-mode`](./skills/poteto-mode/SKILL.md) covers it, but it's not a default.
